/*!
 * @file Adafruit_SSD1306_Wemos.cpp
 *
 * @mainpage [PATCHED by kpal for Wemos support] Arduino library for monochrome OLEDs based on SSD1306 drivers.
 *
...
/*!
    ...
    @return Adafruit_SSD1306_Wemos object.
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_Wemos::Adafruit_SSD1306_Wemos(uint8_t w, uint8_t h, bool is_wemos_oled, TwoWire *twi,
                                   int8_t rst_pin, uint32_t clkDuring,
                                   uint32_t clkAfter)
        : Adafruit_GFX(w, h), isWemosOled(is_wemos_oled), spi(NULL), wire(twi ? twi : &Wire), buffer(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_Wemos::Adafruit_SSD1306_Wemos(uint8_t w, uint8_t h, int8_t mosi_pin,
                                   int8_t sclk_pin, int8_t dc_pin,
                                   int8_t rst_pin, int8_t cs_pin)
        : Adafruit_GFX(w, h), isWemosOled(false), spi(NULL), wire(NULL), buffer(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_Wemos::Adafruit_SSD1306_Wemos(uint8_t w, uint8_t h, SPIClass *spi,
                                   int8_t dc_pin, int8_t rst_pin, int8_t cs_pin,
                                   uint32_t bitrate)
        : Adafruit_GFX(w, h), isWemosOled(false), spi(spi ? spi : &SPI), wire(NULL), buffer(NULL),
...
/*!
    @brief  DEPRECATED constructor for SPI SSD1306 displays, using software
            ...
            one of the SSD1306_* size defines in Adafruit_SSD1306_Wemos.h. New
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_Wemos::Adafruit_SSD1306_Wemos(int8_t mosi_pin, int8_t sclk_pin,
                                   int8_t dc_pin, int8_t rst_pin, int8_t cs_pin)
        : Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), isWemosOled(false), spi(NULL), wire(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_Wemos::Adafruit_SSD1306_Wemos(int8_t dc_pin, int8_t rst_pin, int8_t cs_pin)
        : Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), isWemosOled(false), spi(&SPI), wire(NULL),
...
/*!
    @brief  DEPRECATED constructor for I2C SSD1306 displays. Provided for
            ...
            defines in Adafruit_SSD1306_Wemos.h. New code should NOT use this.
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_Wemos::Adafruit_SSD1306_Wemos(int8_t rst_pin)
        : Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), isWemosOled(false), spi(NULL), wire(&Wire),
          buffer(NULL), mosiPin(-1), clkPin(-1), dcPin(-1), csPin(-1),
          rstPin(rst_pin) {}

/*!
    @brief  Destructor for Adafruit_SSD1306_Wemos object.
*/
Adafruit_SSD1306_Wemos::~Adafruit_SSD1306_Wemos(void) {
    if (buffer) {
        free(buffer);
        buffer = NULL;
    }
...
/*!
   ...
   method
*/
inline void Adafruit_SSD1306_Wemos::SPIwrite(uint8_t d) {
    if (spi) {
        (void)spi->transfer(d);
    } else {
...
/*!
   ...
   protected function, not exposed (see ssd1306_command() instead).
    ...
    @note
*/
void Adafruit_SSD1306_Wemos::ssd1306_command1(uint8_t c) {
    if (wire) { // I2C
...
/*!
   ...
   transactions. This is a protected function, not exposed.
    ...
    @note
*/
void Adafruit_SSD1306_Wemos::ssd1306_commandList(const uint8_t *c, uint8_t n) {
    if (wire) { // I2C
        wire->beginTransmission(i2caddr);
        WIRE_WRITE((uint8_t)0x00); // Co = 0, D/C = 0
        uint16_t bytesOut = 1;
...
/*!
    ...
    @return None (void).
*/
void Adafruit_SSD1306_Wemos::ssd1306_command(uint8_t c) {
    TRANSACTION_START
    ssd1306_command1(c);
    TRANSACTION_END
}
...
*/
bool Adafruit_SSD1306_Wemos::begin(uint8_t vcs, uint8_t addr, bool reset,
                             bool periphBegin) {
...
*/
void Adafruit_SSD1306_Wemos::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void Adafruit_SSD1306_Wemos::clearDisplay(void) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void Adafruit_SSD1306_Wemos::drawFastHLine(int16_t x, int16_t y, int16_t w,
                                     uint16_t color) {
    bool bSwap = false;
    switch (rotation) {
        case 1:
...
/*!
   ...
   SSD1306_INVERSE.
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void Adafruit_SSD1306_Wemos::drawFastHLineInternal(int16_t x, int16_t y, int16_t w,
                                             uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void Adafruit_SSD1306_Wemos::drawFastVLine(int16_t x, int16_t y, int16_t h,
                                     uint16_t color) {
    bool bSwap = false;
    switch (rotation) {
        case 1:
...
/*!
   ...
   SSD1306_INVERSE.
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void Adafruit_SSD1306_Wemos::drawFastVLineInternal(int16_t x, int16_t __y,
...
*/
bool Adafruit_SSD1306_Wemos::getPixel(int16_t x, int16_t y) {
    if ((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
        // Pixel is in-bounds. Rotate coordinates if needed.
        switch (getRotation()) {
            case 1:
...
*/
uint8_t *Adafruit_SSD1306_Wemos::getBuffer(void) { return buffer; }
...
*/
void Adafruit_SSD1306_Wemos::display(void) {
    TRANSACTION_START
//    static const uint8_t PROGMEM dlist1[] = {
//            SSD1306_PAGEADDR,
//            0,                      // Page start address
//            0xFF,                   // Page end (not really, but works here)
//            SSD1306_COLUMNADDR,     // [Patch] This 0 default and 32 for wemos oled
//            };
...
void Adafruit_SSD1306_Wemos::startscrollright(uint8_t start, uint8_t stop) {
    TRANSACTION_START
...
void Adafruit_SSD1306_Wemos::startscrollleft(uint8_t start, uint8_t stop) {
    TRANSACTION_START
...
void Adafruit_SSD1306_Wemos::startscrolldiagright(uint8_t start, uint8_t stop) {
    TRANSACTION_START
...
void Adafruit_SSD1306_Wemos::startscrolldiagleft(uint8_t start, uint8_t stop) {
    TRANSACTION_START
...
/*!
    ...
    @return None (void).
*/
void Adafruit_SSD1306_Wemos::stopscroll(void) {
    TRANSACTION_START
...
*/
void Adafruit_SSD1306_Wemos::invertDisplay(bool i) {
    TRANSACTION_START
...
*/
void Adafruit_SSD1306_Wemos::dim(bool dim) {
